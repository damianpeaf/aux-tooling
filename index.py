...
def filter_files_by_extension(repo_path, valid_extensions):
    ...
    for root, dirs, files in os.walk(repo_path, topdown=False):
        ...
        # Luego leer el archivo si es .cs para saber si es autogenerado por contener patrones comunes de generación automática
        for file in files:
            file_path = os.path.join(root, file)
            ext = os.path.splitext(file)[1].lstrip('.')
            ...
            if ext == "cs":
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        content = f.read(500)  # Leer solo los primeros 500 caracteres para optimizar
                        
                        # Patrón flexible para detectar archivos autogenerados
                        if ("<auto-generated>" in content.lower() and "generated" in content.lower()) or "Generated from" in content:
                            os.remove(file_path)
                            deleted_files += 1
                            auto_generated_files += 1
                except Exception as e:
    ...
    if auto_generated_files > 0:
        print(f"  ⚠️ Se eliminaron {auto_generated_files} archivos autogenerados.")
...
def process_repositories_sequentially(entries, extensions):
    ...
    # Limpiar carpeta de código de referencia base-code/. Con las misma extensiones que los entregados
    print("\nLimpiando carpeta de código de referencia base-code/...")
    base_code_path = 'base-code'
    if os.path.exists(base_code_path):
        deleted_files = filter_files_by_extension(base_code_path, extensions)
        print(f"Se eliminaron {deleted_files} archivos con extensiones no válidas.")
    else:
        print("La carpeta de código de referencia no existe.")
    ...
    try:
        ...
        if entries:
            if save_links_to_csv(list(entries), output_file):
                ...
                # Ejecutar JPlag después de la clonación
                print("Ejecutando JPlag para análisis de similitud...")
                try:
                    subprocess.run(["java", "-jar", "jplag.jar", "repos", "-l", "csharp", "-r", "resultados", "--base-code", "base-code", "--mode", "RUN_AND_VIEW"], check=True)
                    print("JPlag se ejecutó correctamente.")
                except subprocess.CalledProcessError as e:
                    print(f"Error al ejecutar JPlag: {e}")
            else:
